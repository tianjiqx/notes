...
_id: 这是文档的一个唯一标识符。
_seq_no (Sequence Number): 这是一个内部序列号，用于维护索引中的文档版本控制和乐观锁机制。每当你对一个文档进行修改（包括创建、更新或删除），Elasticsearch 会为该文档分配一个新的 _seq_no。这个数字每次操作递增，确保了操作的顺序性，并帮助解决并发修改冲突。在执行更新或删除操作时，可以通过指定 _seq_no 和 _primary_term（另一个内部元数据字段，表示文档在其生命周期中的主要版本）来确保操作基于文档的最新状态，从而避免过时数据的覆盖问题。
