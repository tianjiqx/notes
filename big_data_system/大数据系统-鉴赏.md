...
## 15. Apache Doris/DirisDB

### 15.1 场景

原百度广告业务的实时分析场景业务开发的Palo，捐赠给Apache 更名的Doris。  Doris核心研发团队成立的商业公司，在Doris基础上，扩展开发DorisDB（鼎石数据库）。
...
Apache Druid 是一个实时分析数据库，专为大型数据集OLAP分析而设计。适用于面向时序事件的数据。
