...
- [apache diris 文档](https://doris.apache.org/master/zh-CN/internal/metadata-design.html)
...
hoodie key (record key + partition path)  支持高效的插入。
...
Delta lake 只能单表，hudi看起来是可以支持多表。

hudi 更多旨在处理增量查询，Delta lake 能够比较好的做MR任务，修改原始数据。

iceberg 功能与delta更加接近，并且实现路径也是库。写入也没有延迟。

更多的细节，功能上的对比，可参考ref的博客。

从项目关注度上来说，delta 更多人使用。
